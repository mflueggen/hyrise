...
JitExpression::JitExpression(const std::shared_ptr<JitTupleEntry>& tuple_entry)
    : expression_type{JitExpressionType::Column}, result_entry{tuple_entry}, _result_entry{*tuple_entry} {}

JitExpression::JitExpression(const std::shared_ptr<JitTupleEntry>& tuple_entry, const AllTypeVariant& variant)
    : expression_type{JitExpressionType::Value}, result_entry{tuple_entry}, _result_entry{*tuple_entry}, _variant{variant} {}
...
std::pair<const DataType, const bool> JitExpression::_compute_result_type() {
  const auto& left_tuple_entry = left_child->_result_entry;
  ...
  const auto& right_tuple_entry = right_child->_result_entry;
...
std::optional<ResultValueType> JitExpression::compute(JitRuntimeContext& context) const {
  ...
  if constexpr (std::is_same_v<ResultValueType, bool>) {
    ...
    if (left_child->_result_entry.data_type == DataType::String && !use_value_ids) {
      switch (expression_type) {
        case JitExpressionType::Equals:
...
void JitExpression::update_nullable_information() {
  if (expression_type == JitExpressionType::Column || expression_type == JitExpressionType::Value) {
    _result_entry.is_nullable = result_entry->is_nullable;
    return;
  }
