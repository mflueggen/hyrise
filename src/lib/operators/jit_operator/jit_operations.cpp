...
std::optional<bool> jit_not(const JitExpression& left_side, JitRuntimeContext& context) {
  ...
  DebugAssert(
      (left_side._result_entry.data_type == DataType::Bool || left_side._result_entry.data_type == DataType::Int),
      "invalid type for jit operation not");
  const auto value = left_side.compute<bool>(context);
  if (left_side._result_entry.is_nullable && !value) {
    return std::nullopt;
  } else {
    return !value.value();
  }
...
std::optional<bool> jit_and(const JitExpression& left_side, const JitExpression& right_side,
  ...
  const auto left_entry = left_side._result_entry;
  const auto right_entry = right_side._result_entry;
...
std::optional<bool> jit_is_null(const JitExpression& left_side, JitRuntimeContext& context, const bool use_value_id) {
  ...
  switch (left_side._result_entry.data_type) {
